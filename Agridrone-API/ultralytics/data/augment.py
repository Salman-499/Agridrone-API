...
class BaseMixTransform:
    ...
    def __call__(self, labels):
        ...
        Args:
            labels (Dict): A dictionary containing label data class Mosfor an image.
    ...
    def _update_label_text(labels):
        ...
        assert 0 , 'texts was not in labels when debugged, def _update_label_text() '
...
class Mosaic(BaseMixTransform):
    ...
    def _mosaic4(self, labels):
        ...
        for i in range(4):
            ...
            labels_patch = self._update_labels(labels_patch, padw, padh) # _update_labels() modfied labels['instances'] only
            mosaic_labels.append(labels_patch)
        final_labels = self._cat_labels(mosaic_labels) # although mosaic_labels (a list of length 4) contains growth stage for every lablels it contains, the _cat_labels messes this up 
    ...
    def _cat_labels(self, mosaic_labels):
        ...
        if len(mosaic_labels) == 0:
            return {}
        cls = []
        growth_stage = [] # to accomodate the additional growth_stage
        instances = []
        imgsz = self.imgsz * 2  # mosaic imgsz
        for labels in mosaic_labels:
            cls.append(labels["cls"])
            growth_stage.append(labels["growth_stage"])
        ...
        final_labels = {
            ...
            "growth_stage": np.concatenate(growth_stage, 0), # to accomodate the additional growth_stage
        ...
        final_labels["growth_stage"] = final_labels["growth_stage"][good]
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        labels["growth_stage"] = np.concatenate([labels["growth_stage"], labels2["growth_stage"]], 0) # to accomodate the additional growth_stage
        return labels
...
class RandomPerspective:
    ...
    def __call__(self, labels):
        ...
        img = labels["img"]
        cls = labels["cls"]
        growth_stage = labels["growth_stage"] # to accomodate the additional growth_stage
        ...
        labels["cls"] = cls[i]
        labels["growth_stage"] = growth_stage[i] # to accomodate the additional growth_stage
        labels["img"] = img
...
class CopyPaste(BaseMixTransform):
    ...
    def _transform(self, labels1, labels2={}):
        ...
        im = labels1["img"]
        cls = labels1["cls"]
        growth_stage = labels1["growth_stage"] # to accomodate the additional growth_stage
        h, w = im.shape[:2]
        ...
        for j in indexes[: round(self.p * n)]:
            ...
            growth_stage = np.concatenate((growth_stage, labels2.get("growth_stage", growth_stage)[[j]]), axis=0) # to accomodate the additional growth_stage
        ...
        labels1["img"] = im
        labels1["cls"] = cls
        labels1["growth_stage"] = growth_stage
...
class Albumentations:
    ...
    def __call__(self, labels):
        ...
        if self.transform is None or random.random() > self.p:
            return labels
        # it seems like the boolean var is False and so it isn't called during training. it may not be called during val too
        if self.contains_spatial:
            cls = labels["cls"]
            growth_stage = labels["growth_stage"]
            if len(cls):
                im = labels["img"]
                ...
                new = self.transform(image=im, bboxes=bboxes, class_labels=cls)  # transformed # will have to change self.transform() too but it isn't being called during training for some reason
                assert 'in def __call__ of class Albumentaions, self.transform() called self.contains_spatial was True'
                if len(new["class_labels"]) > 0:  # skip update if no bbox in new im
                    labels["img"] = new["image"]
                    labels["cls"] = np.array(new["class_labels"])
                    labels["growth_stage"] = np.array(new["growth_stage"])
...
class Format:
    ...
    def __call__(self, labels):
        ...
        h, w = img.shape[:2]
        cls = labels.pop("cls")
        growth_stage = labels.pop("growth_stage")
        instances = labels.pop("instances")
        ...
        if self.return_mask:
            if nl:
                masks, instances, cls, growth_stage = self._format_segments(instances, cls, growth_stage, w, h)
        ...
        labels["growth_stage"] = torch.from_numpy(growth_stage) if nl else torch.zeros(nl) # to accomodate the additional growth_stage
    ...
    def _format_segments(self, instances, cls, growth_stage, w, h): # the extra parameters is to accomodate the additional growth_stage
        """
        ...
        if self.mask_overlap:
            ...
            cls = cls[sorted_idx]
            growth_stage = growth_stage[sorted_idx]
        else:
            masks = polygons2masks((h, w), segments, color=1, downsample_ratio=self.mask_ratio)

        return masks, instances, cls, growth_stage # the place where it was being called augment.py:2066 has also been changed to handle this


class RandomLoadText: # since this isn't called during training, it won't be modified
    """
    ...
    def __call__(self, labels: dict) -> dict:
        ...
        # growth_stage = np.asarray(labels.pop("growth_stage"), dtype=float) # to accomodate the additional growth_stage, growth_stage is a float, a regression value
