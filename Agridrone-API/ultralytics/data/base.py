...
class BaseDataset(Dataset):
    ...
    def get_img_files(self, img_path):
        # img_path e.g -> /home/user/Desktop/win_intern/dummy_train/train/images
        """Read image files."""
        try:
            f = []  # image files
    ...
    def __getitem__(self, index):
        ...
        image_and_lb = self.get_image_and_label(index)
        transformed_data = self.transforms(image_and_lb) # change both self.get_image_and_label() and self.transforms
    ...
    def update_labels_info(self, label): # this is overriden elsewhere in class YOLODataset(BaseDataset) so the one below isn't called
