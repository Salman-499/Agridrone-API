...
FORMATS_HELP_MSG = f"hi, how are you Supported formats are:\nimages: {IMG_FORMATS}\nvideos: {VID_FORMATS}"
...
def verify_image(args):
    ...
    try:
        ...
        nc = 1
        msg = f"{prefix}WARNING ⚠️ {im_file}: (error in def verify_image()) ignoring corrupt image/label: {e}"
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file) as f:
                ...
                if any(len(x) > 6 for x in lb) and (not keypoint):  # is segment
                    ...
                    growth_stages = np.array([x[1] for x in lb], dtype=np.float32) # for extra regression scalar value
                    segments = [np.array(x[2:], dtype=np.float32).reshape(-1, 2) for x in lb]  # (cls, xy1...)
                    lb = np.concatenate((classes.reshape(-1, 1), growth_stages.reshape(-1, 1), segments2boxes(segments)), 1)  # (cls, xywh)
                    # assert 0, f'classes: {classes}  growth_stages: {growth_stages}  segments: {segments}  lb: {segments}'
            ...
            if nl := len(lb):
                if keypoint:
                    ...
                    assert lb.shape[1] == 6, f"labels require 5 columns, {lb.shape[1]} columns detected" # '6' for extra regression scalar value
                    # cls, growth_stage, 4 bbox values
                    points = lb[:, 2:] # '2' for extra regression scalar value
                ...
                _, i = np.unique(lb, axis=0, return_index=True) # find unique rows to then remove duplicates
            ...
            nm = 1  # label missing
            lb = np.zeros((0, (5 + nkpt * ndim) if keypoints else 6), dtype=np.float32) # '6' for extra regression scalar value
        if keypoint:
            keypoints = lb[:, 5:].reshape(-1, nkpt, ndim)
            if ndim == 2:
                kpt_mask = np.where((keypoints[..., 0] < 0) | (keypoints[..., 1] < 0), 0.0, 1.0).astype(np.float32)
                keypoints = np.concatenate([keypoints, kpt_mask[..., None]], axis=-1)  # (nl, nkpt, 3)
        lb = lb[:, :6] # # '6' for extra regression scalar value
        return im_file, lb, shape, segments, keypoints, nm, nf, ne, nc, msg
    except Exception as e:
        nc = 1
        msg = f"{prefix}WARNING ⚠️ {im_file}: (error in def verify_image_label()) ignoring corrupt image/label: {e}"
