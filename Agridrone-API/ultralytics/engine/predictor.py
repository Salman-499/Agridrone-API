...
class BasePredictor:
    ...
    def preprocess(self, im): # this preprocess is called during inference
        ...
        # but it IS called when doing inference
        # e.g
        ...
        if not_tensor:
            print(type(im)) # a list of the images you need to preprocess
            print(len(im)) # == to the number of images you want to inference
            print(type(im[0])) # a np.ndarray
            print(f'im[0].shape: {im[0].shape} in def preprocess() predictor.py') # (num_rows, num_cols, num_channels) 
            # ! for some reason the num_channels appear as 3 even though there are 4 in the source image
    ...
    def stream_inference(self, source=None, model=None, *args, **kwargs):
        ...
        with self._lock:  # for thread-safe inference
            ...
            if not self.done_warmup:
                self.model.warmup(imgsz=(1 if self.model.pt or self.model.triton else self.dataset.bs, channels, *self.imgsz))
            ...
            for self.batch in self.dataset:
                ...
                print(f'type(im0s): {type(im0s)}')
                print(f'im0s[0].shape: {im0s[0].shape}')
                # Preprocess
                with profilers[0]:
        ...
        if self.args.verbose and self.seen:
            ...
            LOGGER.info(
                ...
                f"{(min(self.args.batch, self.seen), channels, *im.shape[2:])}" % t
            )
