...
class Results(SimpleClass):
    ...
    def __init__(
        ...
        # added a new growth_val property and changed .....
...
class Boxes(BaseTensor):
    ...
    def __init__(self, boxes, orig_shape) -> None:
        ...
        if boxes.ndim == 1:
            boxes = boxes[None, :]
        n = boxes.shape[-1]
        assert n in {6, 7, 8}, f"expected 6 or 7 or 8 values but got {n}"  # xyxy, track_id, growth_val, conf, cls # changed allowable
        # values for n from 6,7 to 6,7,8
        super().__init__(boxes, orig_shape)
        self.is_track = n == 8 # changed to 8 since box now contains one extra value
        self.orig_shape = orig_shape

    @property
    def growth_val(self):
        """Returns a scalar growth stage regression value"""
        return self.data[:, -3]
    @property
    def xyxy(self):
        """
