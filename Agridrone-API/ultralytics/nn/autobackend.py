...
class AutoBackend(nn.Module):
    ...
    def forward(self, im, augment=False, visualize=False, embed=None): # used during both inference and validation , preds = model(batch["img"], augment=augment) in engine/validator.py calls this
        """
        ...
        # PyTorch
        if self.pt or self.nn_module: # this if statement is executed during validation (on the current config, local + cpu only)
        ...
        if isinstance(y, (list, tuple)):
            ...
            return self.from_numpy(y[0]) if len(y) == 1 else [self.from_numpy(x) for x in y] # this is executed during validation
            # len(y)
            # 2
            # y[0].shape
            # torch.Size([5, 43, 7056])
