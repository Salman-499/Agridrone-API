    ...
    def process_batch(self, detections, gt_bboxes, gt_cls):
        ...
        if gt_cls.shape[0] == 0:  # Check if labels is empty
            if detections is not None:
                detections = detections[detections[:, 5] > self.conf] # changed to '5', was prevously '4'
                detection_classes = detections[:, 6].int() # changed to '6', was previously '5'. to accomodate for the new gs
        ...
        detections = detections[detections[:, 5] > self.conf] # changed to '5', was prevously '4'
        gt_classes = gt_cls.int()
        detection_classes = detections[:, 6].int() # changed to '6', was previously '5'
        is_obb = detections.shape[1] == 8 and gt_bboxes.shape[1] == 6  # with additional `angle` dimension 
        # 7 and 5 changed to 8 and 6 in the above line
        iou = (
