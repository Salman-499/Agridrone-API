...
class Annotator:
    ...
    def masks(self, masks, colors, im_gpu, alpha=0.5, retina_masks=False):
        ...
        im_gpu = im_gpu[:, :, :3] * inv_alpha_masks[-1] + mcs # the image contains 4 channels and we only want the rgb channels, not the .tiff channel
        im_mask = im_gpu * 255
        im_mask_np = im_mask.byte().cpu().numpy()
        self.im[:, :, :3] = im_mask_np if retina_masks else ops.scale_image(im_mask_np, self.im.shape) # the image contains 4 channels and we only want the rgb channels, not the .tiff channel
        if self.pil:
    ...
    def save(self, filename="image.jpg"):
        ...
        cv2.imwrite(filename, np.asarray(self.im[:, :, :3])) # the image contains 4 channels and we only want the rgb channels, not the .tiff channel
...
def save_one_box(xyxy, im, file=Path("im.jpg"), gain=1.02, pad=10, square=False, BGR=False, save=True):
    ...
    # Extract only the first 3 channels (RGB) from the image
    im = im[:, :, :3]  # Keep only the first 3 channels (R, G, B)
...
# images, [batch_id, class_id, x, y, w, h, conf, growth_stage], ........
@threaded
def plot_images(
    ...
    # growth_stage: Union[torch.Tensor, np.ndarray],
    ...
    # Extract only the first 3 channels (RGB) from the image
    images = images[:, :3, :, :]  # Keep only the first 3 channels (R, G, B)
...
def output_to_target(output, max_det=300):
    """Convert model output to target format [batch_id, class_id, x, y, w, h, conf, growth_stage] for plotting."""
    targets = []
    for i, o in enumerate(output):
        box, growth_stage, conf, cls = o[:max_det, :7].cpu().split((4, 1, 1, 1), 1) # was previously box, conf, cls = o[:max_det, :6].cpu().split((4, 1, 1), 1), changed for the additional growth stage
        j = torch.full((conf.shape[0], 1), i)
        targets.append(torch.cat((j, cls, ops.xyxy2xywh(box), conf, growth_stage), 1))
    targets = torch.cat(targets, 0).numpy()
    return targets[:, 0], targets[:, 1], targets[:, 2:-2], targets[:, -2] # this works too if you don't want the growth stage in the output plot
    # return targets[:, 0], targets[:, 1], targets[:, 2:-2], targets[:, -2], targets[:, -1]
    # batch ids ^^^       ^^^ class_ids   ^^^ bboxes        ^^^ confs       ^^^ growth_stages
